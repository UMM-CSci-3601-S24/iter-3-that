...
describe('Join Hunt', () => {
  ...
  it('should navigate back to join hunt page when join hunt is clicked with valid access code and cancel create team', () => {
    cy.visit(`/hosts/`);
    page.getHuntCards().first().then(() => {
      page.clickViewProfile(page.getHuntCards().first());
      cy.url().should('match', /\/hunts\/[0-9a-fA-F]{24}$/);
...
//     // Those above will navigate to the Hunt, begin it
  ...
  it('should navigate to right hunter view page when join hunt is clicked with valid access code and create team', () => {
    cy.visit(`/hosts/`);
    page.getHuntCards().first().then(() => {
      page.clickViewProfile(page.getHuntCards().first());
      cy.url().should('match', /\/hunts\/[0-9a-fA-F]{24}$/);
...
//     // Those above will navigate to the Hunt, begin it
    ...
    page.getCreateTeamButton().click();

    page.getHunterViewTitle().should('exist');
  });
 });
