...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, MatCardModule, MatButtonModule, MatListModule, MatIconModule, RouterLink, MatFormField, MatLabel, MatError, ReactiveFormsModule]
})
export class HuntCardComponent {
  ...
  NumberofTeamsForm = new FormGroup({
    numTeam: new FormControl<number>(null, Validators.compose([
      Validators.required,
      Validators.min(1),
      Validators.max(20),
  ...
  formControlHasError(controlName: string): boolean {
    return this.NumberofTeamsForm.get(controlName).invalid &&
      (this.NumberofTeamsForm.get(controlName).dirty || this.NumberofTeamsForm.get(controlName).touched);
  }

  getErrorMessage(numTeam: keyof typeof this.numberOfTeamsValidationMessages): string {
    for(const {type, message} of this.numberOfTeamsValidationMessages[numTeam]) {
